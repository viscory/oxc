...
impl Gen for SwitchCase<'_> {
    fn r#gen(&self, p: &mut Codegen, ctx: Context) {
        ...
        // Force multi-line if a preserved-comment orphan is pending; the inline path skips the flush.
        let single_line = self.consequent.len() == 1
            && !p.has_preserved_comment_orphans_before(self.consequent[0].span().start);
        if single_line {
...
impl Gen for FunctionBody<'_> {
    fn r#gen(&self, p: &mut Codegen, ctx: Context) {
        ...
        let single_line = if self.is_empty() {
            !p.has_preserved_comment_orphans_before(self.span.end)
                && comments_at_end
                    .as_ref()
...
impl Gen for StaticBlock<'_> {
    fn r#gen(&self, p: &mut Codegen, ctx: Context) {
        ...
        p.print_soft_space();
        let single_line =
            self.body.is_empty() && !p.has_preserved_comment_orphans_before(self.span.end);
        p.print_curly_braces(self.span, single_line, |p| {
