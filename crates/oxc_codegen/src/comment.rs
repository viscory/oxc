...
impl Codegen<'_> {
        ...
        for comment in comments {
            ...
            if add {
                if preserve_when_orphaned(*comment)
                    && let Err(idx) =
                        self.preserved_comment_keys.binary_search(&comment.attached_to)
                {
                    self.preserved_comment_keys.insert(idx, comment.attached_to);
                }
    ...
    /// Whether a preserved comment with an orphaned `attached_to < end` is
    /// still pending. Used by block emitters to keep an empty body multi-line.
    #[inline]
    pub(crate) fn has_preserved_comment_orphans_before(&self, end: u32) -> bool {
        self.preserved_comment_keys
            .iter()
            .take_while(|&&k| k < end)
            .any(|k| self.comments.contains_key(k))
    }

    /// Drain pending preserved-comment orphans with `attached_to < end` and
    /// emit them in source order. Called at every statement boundary so legal
    /// and file-level coverage comments survive when their original anchor was
    /// removed by an upstream pass.
    #[inline]
    pub(crate) fn print_preserved_comment_orphans_before(&mut self, end: u32) {
        if self.preserved_comment_keys.is_empty() {
            return;
        }
        let idx = self.preserved_comment_keys.partition_point(|&k| k < end);
        if idx == 0 {
            return;
        }
        ...
        let mut preserved: Vec<Comment> = Vec::new();
        let comments = &mut self.comments;
        for k in self.preserved_comment_keys.drain(..idx) {
            let Some(entry) = comments.get_mut(&k) else { continue };
            debug_assert!(entry.iter().any(|c| preserve_when_orphaned(*c)));
            preserved.extend(entry.extract_if(.., |c| preserve_when_orphaned(*c)));
            if entry.is_empty() {
                comments.remove(&k);
            }
        }
        if let Some(last) = preserved.last_mut() {
