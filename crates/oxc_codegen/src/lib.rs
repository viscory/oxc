...
pub struct Codegen<'a> {
    ...
    /// removed anchor. Lets `print_preserved_comment_orphans_before` flush via
    /// `partition_point` + `drain`.
    preserved_comment_keys: Vec<u32>,
...
impl<'a> Codegen<'a> {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            preserved_comment_keys: Vec::new(),
            #[cfg(feature = "sourcemap")]
...
impl<'a> Codegen<'a> {
    ...
    fn print_block_statement(&mut self, stmt: &BlockStatement<'_>, ctx: Context) {
        let single_line =
            stmt.body.is_empty() && !self.has_preserved_comment_orphans_before(stmt.span.end);
    ...
    /// Print `stmts`, flushing preserved-comment orphans before each and at `scope_end`.
    fn print_stmts_with_orphan_flush(
        ...
        for stmt in stmts {
            self.print_preserved_comment_orphans_before(stmt.span().start);
            self.print_semicolon_if_needed();
            stmt.print(self, ctx);
        }
        self.print_preserved_comment_orphans_before(scope_end);
    }
    ...
    fn print_directives_and_statements(
        ...
        self.print_preserved_comment_orphans_before(first.span().start);
